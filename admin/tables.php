...
<div class="container">
    ...
    <?php
    $strTable = '<div class="table-responsive"><table class="table table-bordered"><tr bgcolor="#E5E5DF">
<td>ສະໜາມ/ເວລາ</td>';
    ...
    foreach ($arrCourt as $courtNum) {
        $strTable .= '<tr><td id="cc" bgcolor="#06c87488;">' . $courtNum . '</td>'; //Show stadium 1,2
        ...
        foreach ($arrTime as $times) {
            ...
            while ($re_ = mysqli_fetch_array($rs_)) {
                if ($re_['cc'] > 0) { //if booking
                    $strTable .= '<td id="cc" bgcolor="#FF0000">ຈອງແລ້ວ</td>';
                    $run++;
                } else { // if no booking
